Feature: Fight of Cells
  ...
  Scenario: Two level 1 cells fight
    Given a level 1 Ice Cell with 8 life points at position (0,0)
    And a level 1 Fire Cell with 5 life points at position (0,0)
  ...
  Scenario: Two level 3 cells fight
    Given a level 3 Ice Cell with 43 life points at position (1,1)
    And a level 3 Fire Cell with 41 life points at position (1,1)
    When the fight starts
    Then the number of FireCells should be reduced by 1
    And the IceCell should win with 39 life points and level 2

  Scenario: One level 1 IceCell against a level 2 FireCell
    Given a level 1 Ice Cell with 9 life points at position (2,2)
    And a level 2 Fire Cell with 22 life points at position (2,2)
    When the fight starts
  ...
  Scenario: Multiple cells of different levels fight
    Given a level 2 Ice Cell with 25 life points at position (10,10)
    And a level 2 Fire Cell with 22 life points at position (10,10)
    And a level 1 Ice Cell with 12 life points at position (10,10)
    And a level 1 Fire Cell with 15 life points at position (10,10)
    When the fight starts
  ...
  Scenario: A higher-level cell fights against lower-level cells
    Given a level 2 Fire Cell with 30 life points at position (0,0)
    And a level 1 Ice Cell with 15 life points at position (0,0)
    And a level 1 Ice Cell with 15 life points at position (0,0)
    When the fight starts
