...
@given(u'a level {level:d} {team} at position ({row:d}, {column:d}) with {life:d} health points')
...
def step_impl(context, row, column):
    print(str(context.GameController.get_board().get_box(row, column)))
    context.GameController.execute_fusion()

@then(u'a level {level_c1:d} IceCell is created at position ({row:d}, {column:d}) with {life:d} health points and the level {level_c2:d} Ice cells at ({row:d}, {column:d}) disappears from the battlefield')
def step_impl(context, row, column, life, level_c1, level_c2):
    print(str(context.GameController.get_board().get_box(row, column)))
    print(str(context.GameController.get_cells(row, column)[0].get_level()))
    ...
    assert context.GameController.get_cells(row, column)[0].get_level() == Level(level_c1)

@then(u'a level {level_c1:d} FireCell is created at position ({row:d}, {column:d}) with {life:d} health points and the level {level_c2:d} Fire cells at ({row:d}, {column:d}) disappears from the battlefield')
def step_impl(context, row, column, life, level_c1, level_c2):
    assert len(context.GameController.get_cells(row, column)) == 1
...
def step_impl(context, row, column, level_c1, level_c2, life_c1, life_c2):
    ...
    assert context.GameController.get_cells(row,column)[1].get_level() == Level(level_c2)
...
def step_impl(context, row, column, level_c1, level_c2, life_c1, life_c2):
    ...
    assert context.GameController.get_cells(row,column)[1].get_life() == life_c1
    assert context.GameController.get_cells(row,column)[1].get_level() == Level(level_c1)
    assert context.GameController.get_cells(row,column)[0].get_life() == life_c2
    assert context.GameController.get_cells(row,column)[0].get_level() == Level(level_c2)

@then(u'a level {level_c1:d} IceCell is created at position ({row:d}, {column:d}) with {life:d} health points and only one level {level_c2:d} IceCell at ({row:d}, {column:d}) with {life_c2:d} health points disappears from the battlefield')
def step_impl(context, row, column, life, life_c2, level_c1, level_c2):
    print(str(context.GameController.get_cells(row,column)[0].get_life()))
    assert len(context.GameController.get_cells(row, column)) == 2
    assert isinstance(context.GameController.get_cells(row,column)[0], IceCell)
    assert isinstance(context.GameController.get_cells(row,column)[1], IceCell)
    assert context.GameController.get_cells(row,column)[0].get_life() == life
